...
# ]
# ///
"""gemini-longdoc — 장문 문서를 Gemini(대형 컨텍스트, OpenAI 호환 엔드포인트)로 통독시켜
구조화 추출하거나, Kimi-K3와의 2차 교차검증용 자유형 질의에 답하게 하는 MCP 서버.

kimi-longdoc과 동일한 설계 원칙을 따른다(호출 패턴을 그대로 미러링):
...
kimi-longdoc과의 차이:
  - 엔드포인트가 Google Gemini의 OpenAI 호환 레이어(generativelanguage.googleapis.com)다.
  - reasoning_effort 값 범위가 다르다: "none"|"low"|"medium"|"high" (NVIDIA/Kimi는 medium 없음, 대신 max 있음).
  - 인증은 GEMINI_API_KEY 환경변수(또는 Pro/Max 클라우드에서는 claude.ai/code 환경설정의
    API credentials에 generativelanguage.googleapis.com 호스트로 등록한 Bearer 키)를 사용한다.
"""
...
# Gemini 2.5 Pro 컨텍스트 ≈ 1M 토큰 ≈ 3~4M 문자. 여유를 두고 자른다.
DEFAULT_MAX_CHARS = int(os.environ.get("GEMINI_MAX_CHARS", "2800000"))
...
def _auth_headers() -> dict:
    """로컬/Team·Enterprise 클라우드: GEMINI_API_KEY 환경변수로 직접 인증.
    Pro/Max 클라우드: 환경변수를 비워두고 claude.ai/code 환경의 API credential로
    generativelanguage.googleapis.com 을 등록하면, 요청이 세션 밖으로 나간 뒤
    Anthropic 프록시가 헤더를 붙여준다(키가 세션 안에는 전혀 들어오지 않음) —
    그래서 키가 없어도 에러내지 않고 헤더 없이 요청을 보낸다."""
...
def _call(
    ...
    with httpx.Client(timeout=TIMEOUT) as client:
        ...
        if r.status_code in (401, 403):
            raise ToolError(
                "Gemini API 인증 실패(" + str(r.status_code) + "). "
                "로컬/Team·Enterprise 클라우드: GEMINI_API_KEY 환경변수를 확인하세요. "
                "Pro/Max 클라우드: claude.ai/code 환경 설정의 API credentials에 "
                "generativelanguage.googleapis.com 을 등록했는지 확인하세요. "
                + r.text[:500]
            )
        if r.status_code >= 400:
            raise ToolError(
                "Gemini API " + str(r.status_code) + ": " + r.text[:1000]
            )
        data = r.json()
...
def extract_document(
    ...
    """장문 문서를 Gemini 대형 컨텍스트에 통째로 넣어 구조화 데이터로 추출한다.
    ...
    Args:
        ...
        reasoning_effort: "none" | "low" | "medium" | "high".
            지침서·법령 통독처럼 한 번에 정확히 뽑아야 하면 "high".
...
def ask_documents(
    ...
    같은 질문을 Claude/Kimi-K3와 Gemini에 각각 물어 답이 갈리는 지점을 찾는 용도로 쓴다.
...
@mcp.tool()
def list_models(filter: str = "gemini") -> str:
    """Gemini OpenAI 호환 엔드포인트에서 사용 가능한 모델 id 목록을 조회한다.

    정확한 모델 id 표기가 바뀔 수 있으므로, 첫 사용 전 이걸로 확인할 것.
    ...
    with httpx.Client(timeout=httpx.Timeout(30.0)) as client:
        ...
        if r.status_code >= 400:
            raise ToolError("Gemini API " + str(r.status_code) + ": " + r.text[:500])
...
if __name__ == "__main__":
    try:
        ...
        print("gemini-longdoc 기동 실패: " + str(exc), file=sys.stderr)
        raise
