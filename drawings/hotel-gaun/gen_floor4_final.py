...
def unit_dlx_a_loft(tf):
    ...
    로프트(침대)존: 좌측 x0~4700 / 오픈존(입식 라운지+발코니창): 우측 x4700~7000
    사다리는 로프트 경계(x4200~4700)에 밀착, 하부 y0~1200 벽 코너에 배치"""
    W, D = tf.W, tf.D
    # 욕실 (원위치 유지, 좌하단)
    ...
    # 사다리 (로프트 경계 x4200~4700, 하단벽 코너 — 문 스윙 x5900~6800과 완전 이격)
    ladder(tf, 4200, 200, 4700, 1200)
    # 하부(1F) 가구: 로프트 아래 데스크+소파, 오픈존에 엔트리 벤치
    tf.rect("FUR", 2700, 2600, 4100, 3400)                        # 데스크(로프트 아래)
    tf.rect("FUR", 3050, 2750, 3750, 3050)
    tf.rect("FUR", 2700, 3600, 4100, 4700); tf.rect("FUR", 2770, 3670, 4030, 4630)  # 소파(로프트 아래)
    tf.rect("FUR", 200, 3900, 1300, 4700); tf.line("FUR", (200,3900), (1300,4700))  # 옷장(욕실 옆)
    tf.rect("FUR", 4750, 200, 5850, 650)                          # 엔트리 콘솔(오픈존)
    tf.rect("FUR", 4750, 3800, 5850, 4700); tf.rect("FUR", 4820, 3870, 5780, 4630)  # 라운지 체어(오픈존, 창측)
    tf.circle("FUR", (6250, 4250), 260)
    # 오픈보이드(하부 개방) 표기 — 우측 입식 라운지존 (해치만 표시, 실명은 하부에 별도 배치)
    otb_hatch(tf, 4700, 200, 6800, 4800, label=None)
    guardrail(tf, (4700,200), (4700,4800))
    tf.text("오픈존(입식)", (5750, 3050), 150, color=9)
    tf.text("하부 라운지 (층고 2,600)", (3400, 1900), 150, color=9)
    tf.text("① 디럭스 로프트", (1300, 4550), 190)

def unit_dlx_b_loft(tf):
    """디럭스 로프트 내측형 (7000x5000) — 출입: 하단벽, 욕실 원위치(중앙) 유지
    로프트(침대)존: 좌측 x0~3300 / 오픈존(욕실+라운지): 우측 x3300~7000
    사다리는 로프트 경계에 밀착, 상단벽 코너(창측)에 배치 — 침대·소파와 완전 이격"""
    W, D = tf.W, tf.D
    # 욕실 (원위치 유지, 중앙)
    ...
    # 하부(1F) 가구: 로프트 아래 소파+TV, 오픈존(욕실 옆)에 라운지
    tf.rect("FUR", 300, 3200, 1700, 3700)                         # TV장(로프트 아래, 창측)
    tf.rect("FUR", 500, 1600, 2300, 2500); tf.rect("FUR", 570, 1670, 2230, 2430)  # 소파(로프트 아래)
    tf.circle("FUR", (2550, 2050), 250)
    tf.rect("FUR", 5750, 2600, 6800, 3700)                        # 콘솔(오픈존, 욕실 옆)
    tf.rect("FUR", 3600, 3900, 5200, 4700); tf.rect("FUR", 3670, 3970, 5130, 4630)  # 라운지(오픈존, 창측)
    tf.circle("FUR", (5550, 4250), 260)
    otb_hatch(tf, 3400, 200, 6800, 4800, label=None)
    guardrail(tf, (3300,200), (3300,4800))
    tf.text("오픈존(입식)", (6300, 4550), 150, color=9)
    tf.text("② 디럭스 로프트", (1300, 4550), 190)
...
def unit_fam_corner_duplex(tf, wide=False):
    ...
    계단은 입구 옆 우측벽에 밀착(직선 10단), 침대·소파·수납과 완전 이격"""
    W, D = tf.W, tf.D
    # 욕실 (원위치 유지, 좌하단)
    ...
    # 하부(1F) 가구: 부모존 소파 + 다이닝, 계단 아래 콘솔
    tf.rect("FUR", 500, 3000, 3200, 4200); tf.rect("FUR", 570, 3070, 3130, 4130)  # 소파(부모존)
    tf.circle("FUR", (4200, 3600), 350)                                          # 다이닝 테이블
    tf.rect("FUR", W-900, 3600, W-200, 4200)                                     # 콘솔(계단 하부 수납)
    tf.text("부모존 + 거실 (층고 2,200)", (2000, 5900), 190, color=9)
    tf.text("2F 자녀실\n계단 개구부", (W-550, 2400), 140, color=8)
    tf.text("③ 패밀리 복층", (1300, 4700), 190)
...
def south_wing_4f(F):
    ...
    # 유닛 — 45.5 코너 2실은 복층으로 교체, 45.0/리넨은 표준 유지
    ...
    unit_fam45_std(TF(F.x0+12000, F.y0+0, 9000, 5000, my=True))
    unit_linen(TF(F.x0+21000, F.y0+0, 5000, 5000, my=True))
    F.text("패밀리룸(표준)", (16100, 3625), 300); F.text("45.0m² /13.61PY", (16100, 3200), 240)
    F.text("리넨실", (23500, 2700), 340); F.text("25.0m² /7.56PY", (23500, 2200), 240)
    # 패밀리 복층 2실은 유닛 내부 소형 태그(⑤⑥) + 하단 실 스케줄로 표기 (겹침 방지)

def unit_fam45_std(tf):
    """패밀리 45.0 표준형 (9000x5000) — 저심도 비례로 계단 확보 어려워 1차 반영 제외, 표준 유지"""
    W, D = tf.W, tf.D
    ...
    kitchenette(tf, 3400, 5200, 2700, 3300); tf.text("미니주방", (4300, 980), 180)
    wardrobe(tf, 200, 200, 2000, 850, split="v")
    bed_h(tf, 300, 2300, 2300, 4100, head="left"); nightstand(tf, 300, 1750, 800, 2250)
    bed_h(tf, 6800, 2450, 8800, 3550, head="right"); bed_h(tf, 6800, 3700, 8800, 4800, head="right")
    tf.circle("FUR", (4300, 4200), 350)
    tf.rect("FUR", 3550, 3950, 3950, 4350); tf.rect("FUR", 4650, 3950, 5050, 4350)
...
Y0 = -3400
F.text("4F 평면도 (로프트베드/복층 유닛 반영판)", (16500, Y0), 900)
F.text("디럭스 로프트 4실(35.0m²) + 패밀리 복층 2실(45.5m²) + 패밀리 표준 1실(45.0m²) + 리넨실 · 휴게공간 · 발코니", (16500, Y0-1200), 340, color=8)

Y1 = Y0 - 2400
schedule = [
    "① 디럭스 로프트 (외측형)  35.0m² /10.59PY  ×2실 — 1F 라운지+오픈존(층고 2,600/4,500) / 2F 침대다락(층고 1,600)",
    "② 디럭스 로프트 (내측형)  35.0m² /10.59PY  ×2실 — 1F 소파+오픈존(층고 2,600/4,500) / 2F 침대다락(층고 1,600)",
    "③ 패밀리 복층 (코너형)    45.5m² /13.76PY  ×2실 — 1F 부모존+거실(층고 2,200) / 2F 자녀실(층고 2,000)",
    "패밀리룸(표준) 45.0m² /13.61PY ×1실, 리넨실 25.0m² /7.56PY ×1실 — 표준형 유지(아래 주기 5 참조)",
]
...
notes = [
    ...
    "3. [재정리] 사다리·계단은 전 유닛 벽체에 밀착 배치(방 중앙 노출 없음)하고, 개구부·착지존을 침대·소파·수납 가구와 평면상 완전 이격시켜 겹침을 해소했습니다.",
    "4. 욕실·미니주방·출입문 위치는 기존 2~3F 표준형과 동일 좌표를 유지해 급배수 라이저·복도 연결 정합성을 확보했습니다.",
    "5. 패밀리 45.0(9.0×5.0m, 저심도 비례)은 계단 진입 동선 확보가 어려워 이번 1차 반영에서 제외, 표준형(비복층)으로 유지했습니다.",
    "6. 상부(로프트/2F) 평면과 개념단면은 별첨 hotel-gaun-4f-loft-duplex-units.dxf 참조. 이 시트는 4층 전체 평면 반영 결과(하부 기준)입니다.",
]
