...
def unit_dlx_a_loft(tf):
    ...
    # 라운지체어 + 티테이블 (창측, 정적 공간)
    tf.rect("FUR", 5200, 4200, 5900, 4700); tf.rect("FUR", 5270, 4270, 5830, 4630)
    tf.circle("FUR", (4700, 4450), 250)
    # 옷장 (욕실 옆, 통과동선과 무관한 벽면)
    tf.rect("FUR", 200, 3900, 1300, 4700); tf.line("FUR", (200,3900), (1300,4700))
...
def unit_dlx_b_loft(tf):
    ...
    # 라운지 (창측, 정적 공간)
    tf.rect("FUR", 5200, 3900, 6800, 4700); tf.rect("FUR", 5270, 3970, 6730, 4630)
    tf.circle("FUR", (4900, 4250), 260)
...
def unit_fam_corner_duplex(tf, wide=False):
    ...
    # 좌식 소파 + 티테이블 (창측, 계단·주방과 이격된 정적 공간)
    tf.rect("FUR", 3900, 5000, 6300, 5700); tf.rect("FUR", 3970, 5070, 6230, 5630)
    tf.circle("FUR", (3300, 5350), 300)
    tf.text("부모 침실 + 거실 (층고 2,200)", (2000, 5900), 190, color=9)
